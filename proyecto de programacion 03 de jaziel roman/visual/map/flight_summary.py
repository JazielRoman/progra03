...
with tab1:
    ...
    if st.button("▶️ Start Simulation", key="start_sim"):
        if m_edges < n_nodes - 1:
            ...
            # Guardar coordenadas aleatorias de nodos
    ...
    if sim.graph is None:
        ...
        if calc:
            if algoritmo == "DFS":
                ...
                dist, prev = sim.graph.dijkstra(origen_sel)
                costo = dist.get(destino_sel, float("inf"))
                ruta = []
                cur = destino_sel
                ...
                sim.graph.floyd_warshall()
                ruta, costo = sim.graph.get_floyd_path(origen_sel, destino_sel)
...
""")
        ...
        # Botón Complete Delivery manejado fuera del bloque calc
        if st.session_state.get('last_route') and st.button("✅ Complete Delivery and Create Order", key="complete_delivery"):
            ...
            found_order_id = None
            for oid, ord_obj in sim.orders_map.items():
                if (ord_obj.origen_id == origen_r and
                    ord_obj.destino_id == destino_r and
                    ord_obj.status == 'pendiente'):
                    found_order_id = oid
                    break
            if found_order_id is not None:
                sim.complete_delivery(found_order_id, ruta_r, costo_r)
                st.success(f"Orden {found_order_id} completada y registrada en AVL.")
            else:
                st.warning("No hay orden pendiente; se registra ruta directamente.")
                ruta_str = "→".join(str(n) for n in ruta_r)
                nodo_avl = sim.route_avl.search(ruta_str)
                if nodo_avl:
                    nodo_avl.value += 1
                else:
                    sim.route_avl.insert(ruta_str, 1)
            st.session_state.last_route = None
...
with tab4:
    ...
    if sim.graph is None:
        ...
        if rutas:
            ...
            st.dataframe(df)
            edges = sim.get_avl_tree_structure()
            AVLVisualizer.draw_avl_tree(edges)
            st.pyplot(plt)
...
with tab5:
    ...
    if sim.graph is None:
        ...
        if origin:
            df_o = pd.DataFrame([{"Nodo": n, "Visitas": f} for n, f in origin.items()])
            df_o = df_o.set_index("Nodo")
